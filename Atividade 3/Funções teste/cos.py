...
# 4. Treinar o modelo
model.fit(angles_train, cos_values_train, epochs=1000, verbose=0)
...
plt.plot(angles_test, cos_values_true, label='True sin(theta)', color='blue')
plt.plot(angles_test, cos_values_predicted, label='Predicted cos(theta)', color='red')
plt.xlabel('Angle (radians)')
plt.ylabel('cos(theta)')
plt.title('FCNN Interpolation of cos(theta) with TensorFlow/Keras')
plt.legend()
plt.grid(True)
plt.show()
